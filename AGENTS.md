...
- Markup lives in `.tsx` components, one per file, never in a template literal. The JSX runtime is ours and sits in `src/shared/ui`. It is not React and there are no hooks. `renderHtml` gives you a string for the server and the nojs layer, `render` diffs into the DOM, `clear` empties a container. `innerHTML` is for theme templates, plugin HTML that has to run its own scripts, and sanitised markdown. Nothing else.
